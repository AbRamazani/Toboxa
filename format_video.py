...
def open_format_v_page(pre_page):
    ...
    Button(hamburgar_menu, text="چرخاندن ویدئو", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn Medium", 15), command=rotate_v).place(x=0, y=100)
    ...
    Button(hamburgar_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn Medium", 15), command=format_v.destroy).place(x=0, y=550)

    # title and img
    Label(format_v, image=img, bg="light blue").place(x=150, y=0)
    Label(format_v, text="تغییر پسوند", bg="light blue", justify="center", font=("Vazirmatn Medium", 35, "bold")).place(x=325, y=0)
    ...
    def open_vid_page(path):
        ...
        details = LabelFrame(video_detail, text="اطلاعات فایل", bg="light blue", font=("Vazirmatn Medium", 15, "bold"), height=180)
        ...
        file_name = Label(details, text=f"{path.split('/')[-1]} : نام فایل", font=("Vazirmatn Medium", 14, "bold"), bg="light blue").pack()
        file_size = Label(details, text=f"حجم فایل : {convert_size(os.path.getsize(path))}", font=("Vazirmatn Medium", 12), bg="light blue").pack()
        file_time = Label(details, text=f"{convert_time(VideoFileClip(path).duration)} : زمان فایل", font=("Vazirmatn Medium", 12), bg="light blue").pack()
        ...
        format_now = Label(vid_p, text=f"{path.split('/')[-1].split('.')[-1]} : پسوند کنونی", bg="light blue", font=("Vazirmatn Medium", 15, "bold")).pack()

        format_to = Label(vid_p, text=": تبدیل به", bg="light blue", font=("Vazirmatn Medium", 15, "bold")).pack()

        to_format = StringVar()
        to_format_cadr = ttk.Combobox(vid_p, textvariable=to_format, values=types_c, state="readonly", justify="center", font=("Vazirmatn Medium", 10, "bold"))
        to_format_cadr.pack()
        ...
        render = Button(vid_p, text="انجام", bg="light blue", font=("Vazirmatn Medium", 15, "bold"), command=format)
