...
def open_format_v_page(pre_page):
    ...
    def open_hamburger():
        ...
        if mak_ham <= 0:
            ...
            mak_ham += 1
            format_v.after(10, open_hamburger)
    ...
    def close_hamburger():
        ...
        if mak_ham >= -150:
            hamburger_menu.place(x=mak_ham, y=0)
            mak_ham -= 1
            format_v.after(10, close_hamburger)

    hamburger_img = PhotoImage(file="files/images/root/menu.png")
    hamburger_b = Button(format_v, image=hamburger_img, bg="light blue", bd=0, command=open_hamburger)
    hamburger_b.place(x=0, y=0)

    hamburger_menu = Frame(format_v, width=150, height=600, bg="#01ab8c")
    ...
    Button(hamburger_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn", 15), command=format_v.destroy).place(x=0, y=550)
