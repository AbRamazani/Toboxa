...
def open_format_i_page(pre_page):
    ...
    Button(hamburgar_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn Medium", 15), command=format_i.destroy).place(x=0, y=550)

    # title and img
    Label(format_i, image=img, bg="light blue").place(x=150, y=0)
    Label(format_i, text="تغییر پسوند", bg="light blue", justify="center", font=("Vazirmatn Medium", 35, "bold")).place(x=350, y=0)
    ...
    def open_img_page(path):
        ...
        format_now = Label(img_p, text=f"{path.split('/')[-1].split('.')[-1]} : پسوند کنونی", bg="light blue", font=("Vazirmatn Medium", 15, "bold")).pack()

        format_to = Label(img_p, text=": تبدیل به", bg="light blue", font=("Vazirmatn Medium", 15, "bold")).pack()

        to_format = StringVar()
        to_format_cadr = ttk.Combobox(img_p, textvariable=to_format, values=types_c, state="readonly", justify="center", font=("Vazirmatn Medium", 10, "bold"))
        to_format_cadr.pack()
        ...
        render = Button(img_p, text="انجام", bg="light blue", font=("Vazirmatn Medium", 15, "bold"), command=change_format)
