...
def open_date_page(pre_page):
    ...
    Label(date, text="تبدیل تاریخ", bg="light blue", justify="center", font=("Vazirmatn bold", 35)).place(x=300, y=0)
    ...
    Label(cadr, text=": نوع تبدیل", bg="light blue", font=("Vazirmatn bold", 15)).place(x=300, y=0)
    type_con_t = StringVar()
    type_con = ttk.Combobox(cadr, textvariable=type_con_t, values=("شمسی به میلادی", "میلادی به شمسی"), state="readonly", justify="center", font=("Vazirmatn bold", 10))
    type_con.place(x=135, y=5)

    def show_comboboxes(event):
        global day, month, year
        type_s = type_con_t.get()
        Label(cadr, text=": روز", bg="light blue", font=("Vazirmatn bold", 15)).place(x=420, y=40)
        Label(cadr, text=": ماه", bg="light blue", font=("Vazirmatn bold", 15)).place(x=180, y=40)
        Label(cadr, text=": سال", bg="light blue", font=("Vazirmatn bold", 15)).place(x=250, y=80)
        sub.place(x=200, y=120)
        now.place(x=400, y=100)
        if type_s == "شمسی به میلادی":
            day = StringVar()
            r_d = range(1, 32)
            day_s = ttk.Combobox(cadr, textvariable=day, values=tuple(r_d), state="readonly", justify="center", font=("Vazirmatn bold", 10))
            ...
            month_s = ttk.Combobox(cadr, textvariable=month, values=months, state="readonly", justify="center", font=("Vazirmatn bold", 10))
            month_s.place(x=10, y=45)

            year = Entry(cadr, justify="center", font=("Vazirmatn bold", 10))
    ...
    sub = Button(cadr, text="محاسبه کن", bg="light blue", font=("Vazirmatn bold", 15), command=convert)
    now = Button(cadr, text="برو به امروز", bg="light blue", font=("Vazirmatn bold", 8), command=set_now)
    result = Label(cadr, bg="light blue", font=("Vazirmatn bold", 15))
