...
def open_compress_page(pre_page):
    ...
    def open_hamburger():
        ...
        if mak_ham <= 0:
            ...
            mak_ham += 1
            compress.after(10, open_hamburger)
    ...
    def close_hamburger():
        ...
        if mak_ham >= -150:
            hamburger_menu.place(x=mak_ham, y=0)
            mak_ham -= 1
            compress.after(10, close_hamburger)

    hamburger_img = PhotoImage(file="files/images/root/menu.png")
    hamburger_b = Button(compress, image=hamburger_img, bg="light blue", bd=0, command=open_hamburger)
    hamburger_b.place(x=0, y=0)

    hamburger_menu = Frame(compress, width=150, height=600, bg="#01ab8c")
    ...
    Button(hamburger_menu, text="ویرایشگر متن", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn", 15), command=text_editor).place(x=0, y=100)
    Button(hamburger_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn", 15), command=compress.destroy).place(x=0, y=550)
