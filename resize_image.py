...
def open_resize_i_page(pre_page):
    ...
    def open_hamburgar():
        ...
        if mak_ham <= 0:
            ...
            mak_ham += 1
            resize_i.after(10, open_hamburgar)
    ...
    def close_hamburgar():
        ...
        if mak_ham >= -150:
            hamburgar_menu.place(x=mak_ham, y=0)
            mak_ham -= 1
            resize_i.after(10, close_hamburgar)

    hamburgar_img = PhotoImage(file="files/images/root/menu.png")
    hamburgar_b = Button(resize_i, image=hamburgar_img, bg="light blue", bd=0, command=open_hamburgar)
    hamburgar_b.place(x=0, y=0)

    hamburgar_menu = Frame(resize_i, width=150, height=600, bg="#01ab8c")
    ...
    Button(hamburgar_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn Medium", 15), command=resize_i.destroy).place(x=0, y=550)

    # title and img
    Label(resize_i, image=img, bg="light blue").place(x=150, y=0)
    Label(resize_i, text="تغییر اندازه", bg="light blue", justify="center", font=("Vazirmatn Medium", 35, "bold")).place(x=350, y=0)
    ...
    def open_img_page(path):
        ...
        Label(img_p, text="عرض", bg="light blue", font=("Vazirmatn Medium", 14, "bold")).pack()

        w_value = IntVar(value=int(photo.width))
        width = Spinbox(img_p, font=("Vazirmatn Medium", 15, "bold"), textvariable=w_value, from_=1, to=999999999999, wrap=True, command=set_proportional_h)
        width.pack()

        Label(img_p, text="ارتفاع", bg="light blue", font=("Vazirmatn Medium", 14, "bold")).pack()

        h_value = IntVar(value=int(photo.height))
        height = Spinbox(img_p, font=("Vazirmatn Medium", 15, "bold"), textvariable=h_value, from_=1, to=999999999999, wrap=True, command=set_proportional_w)
        height.pack()
        ...
        proportional = IntVar()
        proportional_check = Checkbutton(img_p, text="حفظ تناسب ابعاد", variable=proportional, bg="light blue", font=("Vazirmatn Medium", 15, "bold"))
        ...
        s_b = Button(img_p, text="انجام", bg="light blue", font=("Vazirmatn Medium", 12, "bold"), command=resize).pack(pady=7)
