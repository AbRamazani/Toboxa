...
def open_password_maker_page(pre_page):
    ...
    Button(hamburgar_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn Medium", 15), command=password_maker.destroy).place(x=0, y=550)

    # title and img
    Label(password_maker, image=img, bg="light blue").place(x=150, y=0)
    Label(password_maker, text="تولیدکننده رمز", bg="light blue", justify="center", font=("Vazirmatn Medium", 30, "bold")).place(x=320, y=15)
    ...
    numbers = IntVar()
    numbers_c = Checkbutton(cadr, text="استفاده از اعداد", variable=numbers, bg="light blue", font=("Vazirmatn Medium", 13)).place(x=300, y=0)

    puncts = IntVar()
    puncts_cadr = Checkbutton(cadr, text="(...,#,*)استفاده از علائم", variable=puncts, bg="light blue", font=("Vazirmatn Medium", 13)).place(x=50, y=0)

    Label(cadr, text=": تعداد کاراکتر", bg="light blue", font=("Vazirmatn Medium", 15, "bold")).place(x=335, y=50)
    count = IntVar(value=12)
    count_cadr = Spinbox(cadr, from_=5, to=50, textvariable=count, wrap=True, font=("Vazirmatn Medium", 13, "bold")).place(x=50, y=50, width=280)
