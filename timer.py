...
def open_timer_page(pre_page):
    ...
    def close_hamburger():
        ...
        if mak_ham >= -150:
            hamburger_menu.place(x=mak_ham, y=0)
            mak_ham -= 1
            timer.after(10, close_hamburger)

    hamburger_img = PhotoImage(file="files/images/root/menu.png")
    hamburger_b = Button(timer, image=hamburger_img, bg="light blue", bd=0, command=open_hamburger)
    hamburger_b.place(x=0, y=0)

    hamburger_menu = Frame(timer, width=150, height=600, bg="#01ab8c")
    ...
    Button(hamburger_menu, text="کرنومتر", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn", 15), command=stopwatch).place(x=0, y=65)
    ...
    Button(hamburger_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn", 15), command=timer.destroy).place(x=0, y=550)

    # title and img
    Label(timer, image=img, bg="light blue").place(x=150, y=0)
    Label(timer, text="تایمر", bg="light blue", justify="center", font=("Vazirmatn", 35, "bold")).place(x=365, y=0)
    ...
    h_s = Spinbox(cadr, textvariable=h_val, from_=0, to=24, wrap=True, font=('Vazirmatn',15,'bold'))
    h_s.place(x=0, y=95, width=162, height=45)

    m_val = IntVar(value=0)
    m_s = Spinbox(cadr, textvariable=m_val, from_=0, to=59, wrap=True, font=('Vazirmatn',15,'bold'))
    m_s.place(x=162, y=95, width=162, height=45)

    s_val = IntVar(value=1)
    s_s = Spinbox(cadr, textvariable=s_val, from_=0, to=59, wrap=True, font=('Vazirmatn',15,'bold'))
