...
def open_filter_i_page(pre_page):
    ...
    def open_hamburger():
        ...
        if mak_ham <= 0:
            ...
            mak_ham += 1
            filter_i.after(10, open_hamburger)
    ...
    def close_hamburger():
        ...
        if mak_ham >= -150:
            hamburger_menu.place(x=mak_ham, y=0)
            mak_ham -= 1
            filter_i.after(10, close_hamburger)

    hamburger_img = PhotoImage(file="files/images/root/menu.png")
    hamburger_b = Button(filter_i, image=hamburger_img, bg="light blue", bd=0, command=open_hamburger)
    hamburger_b.place(x=0, y=0)

    hamburger_menu = Frame(filter_i, width=150, height=600, bg="#01ab8c")
    ...
    Button(hamburger_menu, text="تغییر پسوند", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn", 15), command=format_i).place(x=0, y=135)
    Button(hamburger_menu, text="چرخاندن تصویر", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn", 15), command=rotate_i).place(x=0, y=170)
    Button(hamburger_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn", 15), command=filter_i.destroy).place(x=0, y=550)
