...
def open_crop_v_page(pre_page):
    ...
    Button(hamburgar_menu, text="تغییر پسوند", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn Medium", 15), command=format_v).place(x=0, y=65)
    ...
    Button(hamburgar_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("Vazirmatn Medium", 15), command=crop_v.destroy).place(x=0, y=550)

    # title and img
    Label(crop_v, image=img, bg="light blue").place(x=150, y=0)
    Label(crop_v, text="برش ویدئو", bg="light blue", justify="center", font=("Vazirmatn Medium", 35, "bold")).place(x=325, y=0)
    ...
    def open_vid_page(path):
        ...
        Label(vid_p, text="شروع و پایان برش(ثانیه)", bg="light blue", font=("Vazirmatn Medium", 14, "bold")).pack(pady=30)

        start = IntVar(value=0)
        start_b = Spinbox(vid_p, font=("Vazirmatn Medium", 15, "bold"), textvariable=start, from_=0, to=v.vid_player.duration(), wrap=True)
        start_b.pack()

        end = IntVar(value=v.vid_player.duration())
        end_b = Spinbox(vid_p, font=("Vazirmatn Medium", 15, "bold"), textvariable=end, from_=0, to=v.vid_player.duration(), wrap=True)
        end_b.pack()
        ...
        Button(vid_p, text="انجام", bg="light blue", font=("Vazirmatn Medium", 15, "bold"), command=crop).pack()
