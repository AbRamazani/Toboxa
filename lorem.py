...
def open_lorem_page(pre_page):
    ...
    Button(hamburgar_menu, text="فشرده‌سازی\n‌فایل‌های‌وب", width=13, bg="#01ab8c", fg="white", bd=0, font=("vazir", 15), command=compress).place(x=0, y=65)
    Button(hamburgar_menu, text="ویرایشگر متن", width=13, bg="#01ab8c", fg="white", bd=0, font=("vazir", 15), command=text_editor).place(x=0, y=135)
    Button(hamburgar_menu, text="خروج", width=13, bg="#01ab8c", fg="white", bd=0, font=("vazir", 15), command=lorem.destroy).place(x=0, y=550)

    # title and img
    Label(lorem, image=img, bg="light blue").place(x=150, y=0)
    Label(lorem, text="لورم ساز", bg="light blue", justify="center", font=("vazir bold", 35)).place(x=400, y=0)
    ...
    Label(cadr, text=": نوع خروجی", bg="light blue", font=("vazir bold", 15)).place(x=350, y=0)
    type_l = StringVar()
    type_l_cadr = ttk.Combobox(cadr, textvariable=type_l, values=("پاراگراف", "جمله", "کلمه"), state="readonly", justify="center", font=("vazir bold", 10)).place(x=185, y=5)

    is_reandom = IntVar()
    is_reandom_cadr = Checkbutton(cadr, text="تصادفی", variable=is_reandom, bg="light blue", font=("vazir bold", 15)).place(x=50, y=0)

    Label(cadr, text=": زبان خروجی", bg="light blue", font=("vazir bold", 15)).place(x=350, y=50)
    language = StringVar()
    language_cadr = ttk.Combobox(cadr, textvariable=language, values=("français", "English", "فارسی", "العَرَبِيّة", "русский", "Deutsch"), state="readonly", justify="center", font=("vazir bold", 10)).place(x=185, y=55)

    Label(cadr, text=": تکرار", bg="light blue", font=("vazir bold", 15)).place(x=90, y=50)
    repeat = IntVar()
    repeat_cadr = Spinbox(cadr, from_=0, to=100, textvariable=repeat, wrap=True, font=("vazir bold", 13), width=2).place(x=50, y=50)
