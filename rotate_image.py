...
def open_rotate_i_page(pre_page):
    ...
    Label(rotate_i, text="چرخاندن تصویر", bg="light blue", justify="center", font=("Vazirmatn bold", 35)).place(x=315, y=0)
    ...
    def open_img_page(path):
        ...
        Label(img_p, text="توجه : تصویر زیر تنها برای نمایش خروجی است و خروجی با کیفیت اصلی ذخیره می شود", bg="light blue", font=("Vazirmatn bold", 10), fg="red").pack()
        ...
        Label(img_p, text="میزان چرخش(پادساعتگرد)", bg="light blue", font=("Vazirmatn bold", 14)).pack()

        rotate_val = IntVar(value=360)
        rotate_s = Spinbox(img_p, font=("Vazirmatn bold", 15), textvariable=rotate_val, from_=1, to=360, wrap=True, command=rotate_show)
        rotate_s.pack()
        ...
        def option_pro():
            ...
            wn = round(hn / ta)
            photo1 = ImageTk.PhotoImage(image=photo.resize((wn, hn)))
            Label(pro_p, text="توجه : تصویر زیر تنها برای نمایش خروجی است و خروجی با کیفیت اصلی ذخیره می شود", bg="light blue", font=("Vazirmatn bold", 10), fg="red").pack()
            ...
            f_v = Button(pro_p, text="قرینه عمودی", bg="light blue", font=("Vazirmatn bold", 15), command=lambda: change_flip(Image.FLIP_TOP_BOTTOM))
            f_v.pack(side=RIGHT, padx=5, fill=X, expand=True, pady=10)
            f_h = Button(pro_p, text="قرینه افقی", bg="light blue", font=("Vazirmatn bold", 15), command=lambda: change_flip(Image.FLIP_LEFT_RIGHT))
            f_h.pack(side=RIGHT, padx=5, fill=X, expand=True, pady=10)

            render = Button(pro_p, text="انجام", bg="light blue", font=("Vazirmatn bold", 15), command=flip_image)
            back = Button(pro_p, text="بازگشت", bg="light blue", font=("Vazirmatn bold", 15), command=back_chose)

            pro_p.mainloop()

        Button(img_p, text="انجام", bg="light blue", font=("Vazirmatn bold", 15), command=rotate_image).pack()
        Button(img_p, text="گزینه های پیشرفته", bg="light blue", font=("Vazirmatn bold", 15), command=option_pro).pack(pady=10)
